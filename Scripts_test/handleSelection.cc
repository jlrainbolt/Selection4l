...
TH1D* select_mumu(TString rootFile, TString suffix, TH1F *h_mll, TH1F *h_pt1, TH1F *h_pt2,
        TH1F *h_eta1, TH1F *h_eta2, TH1F *h_npv, TH1F *h_n_bad, TH1F *h_bad_pt, TH1F *h_bad_eta);
...
void handleSelection(const TString selection, const TString suffix, const TString id)
{
    ...
    // Histograms
    const unsigned M  = 9;    TString hname[M],             htitle[M];
    ...
    const unsigned PV = 5;    hname[PV] = "nPV";            htitle[PV] = "# Primary Vertices";
    const unsigned X1 = 6;    hname[X1] = "nExtraLeps";     htitle[X1] = "# extra " + lepton+"s";
    const unsigned X2 = 7;    hname[X2] = "extraLepPt";     htitle[X2] = "Extra " + lepton+"s Pt";
    const unsigned X3 = 8;    hname[X3] = "extraLepEta";    htitle[X3] = "Extra " + lepton+"s Eta";
    ...
    bins[X1] = 7;       low[X1] = -0.5;     up[X1] = 6.5;
    bins[X2] = 75;      low[X2] = 0;        up[X2] = 75;
    bins[X3] = 50;      low[X3] = -5;       up[X3] = 5;
        ...
        TH1F *h_eta1, TH1F *h_eta2, TH1F *h_npv, TH1F *h_n_bad, TH1F *h_bad_pt, TH1F *h_bad_eta)
{
    ...
    TTreeReaderValue<std::vector<Bool_t>> muonIsPF_(reader, "muonIsPFMuon");
...
//  reader.GetTree()->Print();
    ...
    while (reader.Next())
    {
        ...
        vector<Int_t> muonQ;
        vector<Float_t> muonTrkIso, muonSF, muonIDEff, muonIsoEff, muonTrigEffData, muonTrigEffMC;
        vector<Bool_t> muonIsStd, muonIsPF;
        UInt_t nMuons = *nMuons_, nStdMuons = *nStdMuons_;

        // For selection
        vector<Bool_t> muonIsIso;
        Int_t nIsoMuons = 0;
        ...
        if (passTrigger && nStdMuons == 2)// && nMuons == 2)
        {
            ...
            for (unsigned i = 0; i < nMuons; i++)
            {
                if (muonTrkIso[i] / muonP4[i].Pt() < ISO_MAX)
                {
            ...
            for (unsigned j = 1; j < nMuons; j++)
            {
                        ...
                        && muonIsPF[0] && muonIsPF[j])
                {
                    ...
                    if (mll > MLL_MIN && mll < MLL_MAX)
                    {
                        ...
                        {
                            if (!muonIsStd[k] && muonIsIso[k])
                            {
                                h_bad_pt->Fill(muonP4[k].Pt(), eventWeight);
                                h_bad_eta->Fill(muonP4[k].Eta(), eventWeight);
                            }
                        }
                        ...
                        && elecIsStd[0] && elecIsStd[j]) 
                {
                    ...
                    if (mll > MLL_MIN && mll < MLL_MAX)
                    {
                        ...
                        for (unsigned k = 0; k < nElecs; k++)
                        {
                            if (!elecIsStd[k])
                            {
                                h_bad_pt->Fill(elecP4[k].Pt(), eventWeight);
                                h_bad_eta->Fill(elecP4[k].Eta(), eventWeight);
                            }
                        }
