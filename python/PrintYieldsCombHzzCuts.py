...
for sel_ in ["4l"]:
    fmt = '{:,.1f}'
    selection = sel_4l
    categories = ["Non", "VV", "VVV", "ttZ"]

    fileName = "Yield" + selDef[sel_] + "Sum" + "_" + cutstr + ".tex"
    f = open(fileName, "w")

    if sel_ == "ll":
        f.write(r"\begin{tabular}{l l l r@{ $\pm$ }r r@{ $\pm$ }r c r@{ $\pm$ }r}" + "\n")
    else:
        f.write(r"\begin{tabular}{l l l r@{ $\pm$ }l r@{ $\pm$ }l r@{ $\pm$ }l c r@{ $\pm$ }l}" + "\n")
    f.write(r"\toprule" + "\n")
