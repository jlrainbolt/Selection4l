...
hnames = ["zzm_350", "zzm_500", "zzm_1000", "zzm_60_120", "zzm_70_110", "zzm_75_105"]
...
mc_arr, bg_arr = np.empty((N_MC, H), dtype=T), np.empty((N_MC, H), dtype=T)
mc, bg = {}, {}
h, j = 0, 0
year = "2018"
...
for suff in mc_suff[year]:
    ...
    bgName = "bkg_" + prefix + "_" + year + "_" + suff + ".root"
    bgFile = TFile.Open(bgName)
    print("Opened", bgName)
    ...
    for sel in selection:
        for hname in hnames:
            ...
            bg_arr[j][h][sel] = bgFile.Get(sel + "/" + hname + "_" + suff)
            bg_arr[j][h][sel].SetDirectory(0)
            bg_arr[j][h][sel].Scale(sf)
...
#               hist = bgFile.Get(sel + "/" + hname + "_" + suff)
#               hist.SetDirectory(0)
#               hist.Scale(sf)
#               bg_arr[j][h][sel].Add(hist)

                h = h + 1
            h = 0


    mc[suff], bg[suff] = mc_arr[j], bg_arr[j]
    j = j + 1
    inFile.Close()
