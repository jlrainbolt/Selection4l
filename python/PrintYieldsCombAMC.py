...
mc_suff = {}
from Cuts2016 import MC_SUFF_AMC
MC_SUFF_AMC.remove("zz_4l")
mc_suff["2016"] = MC_SUFF_AMC
from Cuts2017 import MC_SUFF_AMC
MC_SUFF_AMC.remove("zz_4l")
mc_suff["2017"] = MC_SUFF_AMC
from Cuts2018 import MC_SUFF_AMC, MC_TEX
MC_SUFF_AMC.remove("zz_4l")
mc_suff["2018"] = MC_SUFF_AMC
...
for sel in sel_4l:
    ...
    fileName = "Yield" + selDef[sel] + "AMC.tex"
    f = open(fileName, "w")

    f.write(r"\begin{tabular}{lll r@{ $\pm$ }r r@{ $\pm$ }r r@{ $\pm$ }r}" + "\n")
    f.write(r"\toprule" + "\n")

    f.write("\t" + r"& & & \multicolumn{6}{c}{$N_{" + selTeX[sel] + r"}$ (events)} \\" + "\n")
    f.write(r"\cmidrule{4-9}" + "\n")
...
for year in period:
    for sel in selection:
        ...
        for suff in mc_suff[year]:
            if suff in ["zz_4l_aMC", "ww_2l2nu", "wz_2l2q", "wz_3lnu", "zz_2l2nu", "zz_2l2q"]:
...
for sel_ in ["4l"]:
    ...
    fileName = "Yield" + selDef[sel_] + "SumAMC.tex"
    f = open(fileName, "w")
