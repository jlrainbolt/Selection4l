...
hnames = ["zzm_70_170_HZZ", "zzm_70_500_HZZ", "zzm_70_HZZ"]
cut2 = ["(zzp4.M() < 170)", "(zzp4.M() < 502)", "(zzp4.M() < 1070)"]

cut = "(((fabs(z2p4.M() - 91.2) < fabs(z1p4.M() - 91.2)) * (z2p4.M() > 40) * (z2p4.M() < 120) * (z1p4.M() > 12) * (z1p4.M() < 120)) || ((fabs(z2p4.M() - 91.2) > fabs(z1p4.M() - 91.2)) * (z1p4.M() > 40) * (z1p4.M() < 120) * (z2p4.M() > 12) * (z2p4.M() < 120))) * (zzp4.M() > 70)"
...
for h in range(H):
    ...
    for year in period:
        ...
        for sel in selection:
            ...
            npt_[year][sel] += muBkgTree.GetEntries("(nLooseLeptons == 0) * " + cut + " * " + cut2[h])
            npt_[year][sel] += elBkgTree.GetEntries("(nLooseLeptons == 0) * " + cut + " * " + cut2[h])
        ...
        for suff in mc_suff[year]:
            if suff in ["zjets_m-50", "ttbar", "tt_2l2nu"]:
                ...
                npt_[year][sel] -= sf * bkgTree.GetEntries("(nLooseLeptons == 0) * " + cut + " * " + cut2[h])
                bkgFile.Close()
    npt[hnames[h]] = npt_
...
#           top_max = 1.3 * top_max
        ...
        if hnames[h] in ["zzm_70_500_HZZ", "zzm_70_HZZ"]:
            plt.xscale("log")
...
#       else:
        ...
        ax_top.text(0.025,  0.875,  "Work in Progress",
                ...
                verticalalignment = 'top', transform = ax_top.transAxes, usetex = False)
        ax_top.set_title(r'\Large{59.7\,fb$^{-1}$ (13\,TeV, 2018)', loc='right')
        ...
        fig_name = "hzz_" + hnames[h] + "_" + sel + "_2018.pdf"
