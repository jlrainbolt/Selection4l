...
for suff in MC_SUFF:
    ...
    if suff == "zz_4l_aMC":
        inFile = TFile.Open(EOS_PATH + "/Selected/" + YEAR_STR + "_update/" + inName)
    else:
    ...
    for sel in selection:
        ...
        # Get signal
        if (suff == "zz_4l_aMC" and sel in ["4l", "4m", "2m2e", "4e"]) or (suff == "zjets_m-50" and sel in ["mumu", "ee"]):
...
fileName = "Yield" + YEAR_STR + "AMC.tex"
f = open(fileName, "w")
...
for suff in MC_SUFF:
    if suff == "tt_2l2nu":
        continue
    if suff == "zz_4l_aMC" and sel in ["4l", "4m", "2m2e", "4e"]:
        f.write("\t&\t&\t" + r"$\ZZtofL$")
...
print("")
outfile = "yields" + YEAR_STR + "AMC.npz"
