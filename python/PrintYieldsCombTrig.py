...
for year in period:
    ...
    for sel in selection:
        ...
        if sig[year][sel] > 0:
            ...
            pur_unc_[sel]   = pur_[sel] * exp_unc[year][sel] / exp[year][sel]
...
for sel in sel_4l:
    ...
    fileName = "Yield" + selDef[sel] + cut + ".tex"
    f = open(fileName, "w")
...
for sel_ in ["4l"]:
    ...
    fileName = "Yield" + selDef[sel_] + "Sum" + cut + ".tex"
    f = open(fileName, "w")

    if sel_ == "ll":
        f.write(r"\begin{tabular}{l l l r@{ $\pm$ }r c r@{ $\pm$ }r r@{ $\pm$ }r}" + "\n")
    else:
        f.write(r"\begin{tabular}{l l l r@{ $\pm$ }l c r@{ $\pm$ }l r@{ $\pm$ }l}" + "\n")
    f.write(r"\toprule" + "\n")
