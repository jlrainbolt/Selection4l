...
void overlayResults(const TString inFile, const TString selection)
{  
    TString titleTag = "";
    ...
    mcColor = {kBlue, kGreen};
    vector<Int_t> mcStyle = {kDashed, kSolid};
    ...
    for (int i = 0; i < dataSubdir.size(); i++)
    {
        for (unsigned h = 0; h < hname.size(); h++)
        {
            ...
            if (i == 0)
            {
                ...
                dataStack[h]->SetTitle(title + titleTag);
            }
            if (h == 0)
    ...
    {   
        unsigned j = j_ - 1;
        TH1* hTotalEvents;
        mcSubdir[j]->GetObject("TotalEvents_dy_m-50", hTotalEvents);
        Float_t ngen = hTotalEvents->GetBinContent(1) - 2*hTotalEvents->GetBinContent(10);
        Float_t weight = lumi * 1000. * xsec[j] / ngen;
        for (unsigned h = 0; h < hname.size(); h++)
        {
            TH1* hist;
            mcSubdir[j]->GetObject(hname[h] + "_dy_m-50", hist);
            hist->SetLineColor(mcColor[j]); hist->SetLineWidth(2); hist->SetLineStyle(mcStyle[j]);
            hist->Scale(weight);    mcStack[h]->Add(hist);

            if (j == 0)
                mcStack[h]->SetTitle(hist->GetTitle());
            if (h == 0)
                legend->AddEntry(hist, mcSuffix[j], "L");
        }
    }
    ...
    for (unsigned h = 0; h < hname.size(); h++)
    {
        ...
        {
            dataStack[h]->Draw("E");
            mcStack[h]->Draw("HIST NOSTACK SAME");
        }
        else
            mcStack[h]->Draw("HIST NOSTACK");
